...
public interface IFmiComponent
{
	...
	public abstract Fmi2Status setDebugLogging(boolean loggingOn,
			String[] categories) throws FmuInvocationException;
	...
	FmuResult<double[]> getRealOutputDerivatives(long[] scalarValueIndices,
			int[] order) throws FmuInvocationException;
	 ...
	 *            Argument currentCommunicationPoint is the current communication point of the master (𝑡𝑐𝑖)
	 * @param communicationStepSize
	 *            argument communicationStepSize is the communication step size (ℎ𝑐𝑖). The latter must be {@code > 0.0.} The
	 *            slave must integrate until time instant 𝑡𝑐𝑖+1 = 𝑡𝑐𝑖 + ℎ𝑐
	 * @param noSetFMUStatePriorToCurrentPoint
	 *            Argument noSetFMUStatePriorToCurrentPoint is fmi2True if fmi2SetFMUState will no longer be called for
	...
	public abstract Fmi2Status setBooleans(long[] scalarValueIndices,
			...
			FmiInvalidNativeStateException;
