...
static fmi2String logCategoriesNames[] =
...
void allocate_mem(fmi2Real*** arr, fmi2Real n, fmi2Real m, const fmi2CallbackFunctions *functions)
{
	*arr = (fmi2Real**) functions->allocateMemory(n, sizeof(fmi2Real*));
	int i;
	for (i = 0; i < n; i++)
		(*arr)[i] = (fmi2Real*) functions->allocateMemory(m, sizeof(fmi2Real));
}

void deallocate_mem(fmi2Real*** arr, fmi2Real n, const fmi2CallbackFunctions *functions)
{
	int i;
	for (i = 0; i < n; i++)
		functions->freeMemory((*arr)[i]);
	functions->freeMemory(*arr);
}
		...
		fmi2Boolean loggingOn)
{
	...
	if (comp)
	{
		...
		allocate_mem(&(comp->derivatives), MAX_DERIVATIVE_ORDER, NUMBER_OF_REALS, functions);
	...
	if (!comp || !comp->r || !comp->i || !comp->b || !comp->s || !comp->isPositive || !comp->instanceName
			|| !comp->GUID)
	{
		...
		fmi2Boolean stopTimeDefined, fmi2Real stopTime)
{
	...
	if (invalidState(comp, "fmi2SetupExperiment", MASK_fmi2SetupExperiment))
		return fmi2Error;
	FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL, "fmi2SetupExperiment: toleranceDefined=%d tolerance=%g", toleranceDefined,
			tolerance)
...
void fmi2FreeInstance(fmi2Component c)
{
	...
	if (comp->derivatives)
		deallocate_mem(&(comp->derivatives), MAX_DERIVATIVE_ORDER, comp->functions);
	if (comp->isPositive)
		...
		for (i = 0; i < nCategories; i++)
		{
			...
			if (!categoryFound)
			{
				comp->functions->logger(comp->componentEnvironment, comp->instanceName, fmi2Warning,
						logCategoriesNames[LOG_ERROR], "logging category '%s' is not supported by model",
						categories[i]);
			}
		}
	}
...
fmi2Status fmi2SetString(fmi2Component c, const fmi2ValueReference vr[], size_t nvr, const fmi2String value[])
{
	...
	for (i = 0; i < nvr; i++)
	{
		...
		{
			if (string)
				comp->functions->freeMemory(string);
			comp->s[vr[i]] = NULL;
...
//    comp->r = (fmi2Real *)   functions->allocateMemory(NUMBER_OF_REALS,    sizeof(fmi2Real));
	memcpy(r, comp->r,  NUMBER_OF_REALS * sizeof(fmi2Real));

	*FMUstate = r;

	printf("state ptr %p\n",r);

	FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL, "fmi2GetFMUstate -- This is only partial for testing (only reals are handled)")
...
{int i = 0;
	...
	FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL, "fmi2SetFMUstate -- This is only partial for testing (only reals are handled)")
	printf("state ptr %p\n",r);
	...
	memcpy( comp->r, r, NUMBER_OF_REALS * sizeof(fmi2Real));
...
fmi2Status fmi2FreeFMUstate(fmi2Component c, fmi2FMUstate* FMUstate)
{
	...
	FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL, "fmi2FreeFMUstate -- This is only partial for testing (only reals are handled)")


	fmi2Real *r =	*FMUstate;

	printf("state ptr %p\n",r);
	comp->functions->freeMemory(r);
		...
		const fmi2ValueReference vKnown_ref[], size_t nKnown, const fmi2Real dvKnown[], fmi2Real dvUnknown[])
{
	...
	for (i = 0; i < nKnown; i++)
	{
		dvUnknown[i] = vKnown_ref[i] * dvKnown[0];
	}
	return fmi2OK;
}
		...
		const fmi2Integer order[], const fmi2Real value[])
{
	...
	{
		for (i = 0; i < nvr; i++) comp->derivatives[order[0]][vr[i]]=value[i];
	}
		...
		const fmi2Integer order[], fmi2Real value[])
{
	...
	// for(j=0;j<MAX_DERIVATIVE_ORDER ; j++ )
	{
		for (i = 0; i < nvr; i++) value[i]=comp->derivatives[order[0]][vr[i]];
	}
	return fmi2OK;
}
...
{
	const char *statusKind[3] =
	{	"fmi2DoStepStatus","fmi2PendingStatus","fmi2LastSuccessfulTime"};
	ModelInstance *comp = (ModelInstance *)c;
